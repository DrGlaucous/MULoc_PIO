...
% Phase angle estimates of UWB tag
...
for i = 1:anchor_num
    ...
    %use a moving average filter to smooth all clock drift entries
    cfo_dw1000(i,:) = smooth(cfo_dw1000(i,:),10);


    %see page 8 of the research paper, it outlines coarse and fine grained estimation
...
% Debug
%carrier frequency offsets with clock drift estimation
figure;
for i = 1:anchor_num
    subplot(2, 2, i);
    ...
    title_str = sprintf('Anchor: %d', i)
    title(title_str)
    legend('cfo dw1000','cfo coarse','cfo fine');
end
S  = axes( 'visible', 'off', 'title', 'carrier frequency offsets with clock drift estimation' );


%% Phase recovery (we calculate this from our carrier frequency offsets)
...
% Remove outlier samples (again, in a similar way to what was seen above)
for i = 1:anchor_num-1
    for j = 1:5
...
for i = 1:anchor_num-1
    ...
    title_str = sprintf('Anchor: %d', i);
    legend('phase rec ch1','phase rec ch3');
    title(title_str);
end
S  = axes( 'visible', 'off', 'title', 'Phase Angle Recovery' );
...
title('recovered time-of-flight');
legend('TOF - 12', 'TOF - 13', 'TOF - 14')
...
plot(d_phase_ch1');hold on
plot(d_phase_ch3');hold on
title('distance difference estimation with each frequency');
legend('tof rec sm','d phase diff', 'd phase f1', 'd phase f2');
